...
\end{itemize}
\item Decisions may cause foreign exchange prices to differ from the efficient outcome
\end{itemize}
\end{frame}
...
\pause
\begin{itemize}[<+-| alert@+>]
\item Even though I may know that the exchange rate is over-valued at $P_t$
\begin{itemize}
...
\begin{frame}{Example:Carry trade and UIP}
A very good example of this is the \emph{carry trade}
\pause
\pause
\begin{itemize}[<+-| alert@+>]
\item This is based on deviations from Uncovered Interest Parity (UIP)
\item Purchase the low interest rate \emph{funding currency} (Yen, EUR, CHF) and buy high yielding \emph{investment currency} (HUF, RUB, ZAR)
\item Depends on no or limited appreciation of the funding currency against the investment currency. 
\item Returns are characterised by a large, negative skew
\item Picking up nickels in front of a steam roller
\end{itemize}
\end{frame}

%put the carry slies here. 
...
\draw [<->, thick] (-5, 0) -- (5, 0); 
\draw [<->, thick] (0, 5) -- (0, -5); 
\draw [<->, thin] (0, 0) to [bend left = 35] (5, 5);
\draw [<->, thin] (0, 0) to [bend left = 35] (-5, -5);
\node [below right] at (0, 5) {$U$}; 
\node [above] at (5, 0) {$\Delta W$};
\end{tikzpicture}
\label{figref:pt}
...
\item Examples, 30\% computer science and 70\% finance but a shy, quiet, socially awkward person is deemed more likely to be a CS student
\end{itemize}
\end{itemize}
\end{frame}
...
x2 <- table(da2)
mosaicplot(x2, main = "Blue and Green Cabs correct and incorrect identification",              xlab = "Cab colour", ylab = "Identification", color = colours)
@
\end{frame}
...
\item UK fund manager Tony Dye was fired in March 2000 for being under-weight technology stocks
\end{itemize}
\end{frame}

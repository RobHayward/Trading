...
There is a \href{http://en.wikipedia.org/wiki/Ornstein-Uhlenbeck_process}{Wikipedia report} on the Ornstein-Uhlenbeck process. An Ornstein-Uhlenbeck process $x_1$ staisfied the following stochastic difference equation 
\begin{equation}
dx_t = \theta(\mu = x_t)dt + \sigma dW_t
\end{equation}

where $\theta > 0$, $\mu \text{and} \sigma > 0$ are parameters and $W_t$ represents a \emph{Wiener Process}. This is like a random walk but there is a drift back towards the average value $\mu$. This is a continuous-time version of an AR(1) process 
...
It is possible that some price series are non-stationary but are stationary when combined with other asset prices.  In that case it is possible  to test for \emph{contegration}.  This will allow the hedge ratio to be uncovered. The technique would be Engle-Granger \citet{engle1987co}. 

This will be tested with two MSCI ETFs for Australia and Canada.  As they are each commodity producers, there should be some realtionship. 
...
@
The regresson of the price levels will provide the \emph{Optimal hedge ratio}.  
<<etf3, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4.5, results='asis'>>=
eq1 <- lm(da$EWAC ~ da$EWCC)
xtable(summary(eq1))
@

The residuals from the regresssion are do not appears as stationary here as they do in the text. 

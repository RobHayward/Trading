...
library(urca)
library(xtable)
test <- ur.df(da$lCAD, type = "none", selectlags = "AIC")
testtable <- data.frame(test = test@teststat, Cv = test@cval)
...
There is a good \href{http://quant.stackexchange.com/questions/7666/using-variance-ratios-to-test-for-mean-reversion}{stackoverflow} note on the variance ratio test. The package \lstinline{vrtest} can provide support. The \lstinline{Lo.Mac} function will compute the M1 and M2 statistics from \citet{lLoMacKinlay1988}.  The package also contains a large number of other tests. 
<<lomc, echo=FALSE, warning=FALSE, message=FALSE>>=
library(vrtest)
a <- Lo.Mac(da$lCAD, c(2, 5, 10))
a
@
